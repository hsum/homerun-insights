...
def get_pull_launch_angle_relationship(year: int = Path(..., gt=0)):
...
def get_pull_exit_velocity_relationship(year: int = Path(..., gt=0)):
...
def stats(year: int, stat: str):
    ...
    if year <= 0:
        click.echo("Error: Year must be greater than 0")
        sys.exit(1)
